...
def get_expected_output(input_path):
    ...
    if input_path == 'tests/assets/workflow_docker.txt':
        result = [
            ['docker', 'run', '--rm', '-u', '$(id', '-u)', '-v', '$PWD:/data', '-w', '/data', '--', 'ocrd/all:maximum', '\\'],
            ['ocrd', 'process', '\\'],
            ['"', 'cis-ocropy-binarize', '-I', 'OCR-D-IMG', '-O', 'OCR-D-BIN', '"', '\\'],
            ['"', 'anybaseocr-crop', '-I', 'OCR-D-BIN', '-O', 'OCR-D-CROP', '"', '\\'],
            ['"', 'skimage-binarize', '-I', 'OCR-D-CROP', '-O', 'OCR-D-BIN2', '-P', 'method', 'li', '"', '\\'],
...
def test_workflow_with_docker():
    """E2E test for an OCR-D workflow based on Docker
    """
    input_path = 'tests/assets/workflow_docker.txt'
    result = OCRD_Validator().validate_ocrd_file(input_path)

    assert result == get_expected_output(input_path)
